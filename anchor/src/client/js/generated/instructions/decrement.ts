...
import { FIRSTPROJECT_PROGRAM_ADDRESS } from '../programs';
...
export type DecrementInstruction<
  TProgram extends string = typeof FIRSTPROJECT_PROGRAM_ADDRESS,
  TAccountFirstproject extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountFirstproject extends string
        ? WritableAccount<TAccountFirstproject>
        : TAccountFirstproject,
      ...TRemainingAccounts,
    ]
  >;
...
export type DecrementInput<TAccountFirstproject extends string = string> = {
  firstproject: Address<TAccountFirstproject>;
};

export function getDecrementInstruction<
  TAccountFirstproject extends string,
  TProgramAddress extends Address = typeof FIRSTPROJECT_PROGRAM_ADDRESS,
>(
  input: DecrementInput<TAccountFirstproject>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountFirstproject> {
  // Program address.
  const programAddress = config?.programAddress ?? FIRSTPROJECT_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    firstproject: { value: input.firstproject ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.firstproject)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountFirstproject>;
